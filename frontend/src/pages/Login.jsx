...
function Login() {
    ...
    const HandleSubmit = async(e) =>{
        e.preventDefault();
        const {data} = await axiosInstance.post("/login",{email:email,password:password})
        console.log(data)
        if(data.status === true){
            navigate("/")
        }
    ...
    <div className="flex justify-center items-center bg-gray-100 min-h-screen">
      <div className="bg-white p-8 rounded-lg w-96 shadow-md">
        ...
        <form onSubmit={HandleSubmit}>
          ...
          <button type='submit' className='py-2 px-4 transition-colors rounded-md bg-blue-500 hover:bg-blue-700'>Login</button>
        </form>
      </div>
    </div>
  );
